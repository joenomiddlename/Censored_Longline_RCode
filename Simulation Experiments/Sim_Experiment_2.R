    ...
    for(j in 1:length(mean_attract))
    {
      for(k in 1:dim(saturation_effects)[2])
      {
        ...
        if(mean_attract[j] == 'linear')
        {
          ...
          # compute summary statistics for the temporal fixed effects
          ...
          # Compute bias, MSE, etc., by using the inla.emarginal function
          # NOTE: RMSE actually computes MSE (incorrect name used throughout)
          # NOTE 2: These measures are checking the model's ability to estimate the number of attracted fish
          # We later ignore these measures since longline data can only estimate relative abundance
          ...
          # Next, we refit the CPUE method with a different parametrization to get at relative abundance
          ...
          # Again - check the model's ability to infer absolute abundance (futile)
          ...
          mod5 <- inla(formula = inla.mdata(cbind(bites,low,high)) ~ -1 + factor(station) + f(event_ID, constr=T, model='iid'),
                       ...
                       data= dat)
          # infer the ability to estimate the absolute abundance (futile)
          ...
          mod7 <- inla(formula = inla.mdata(cbind(bites,low,high)) ~ -1 + factor(station) + f(event_ID, constr=T, model='iid'),
                       family="cenpoisson2",verbose=F,
                       data= dat)
          # Estimate absolute abundance (futile)
